...
function useOwnMessageInfo(id: string) {
  const select = useMemo(() => memoize((s: Chat) => messageInfo(s, id)), [id]);
  return useChatStore(select);
}

// With the bug on, every row shares one selector instead.
const useMessageInfo = bug('memo-cache-slot') ? useSharedMessageInfo : useOwnMessageInfo;
