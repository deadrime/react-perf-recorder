...
and the textbook mistakes on bare pages of their own (`/basics/memo`, `/basics/keys`), each showing the broken and
the fixed version of one widget side by side. `/` lists them as cards that say what to do on the page and what the recording should name, so a recording can
